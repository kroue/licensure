...
function normalizeForSearch(value: string): string {
  return repairStudentText(value)
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/ñ/g, 'n')
    .replace(/Ñ/g, 'N')
...
function toDisplayRow(source: PredictionRowAnalytics, index: number): DisplayRow {
  ...
  const studentId = repairStudentText(student['Student_Code'] ?? student['studentId'] ?? `row-${index + 1}`)
  const name = repairStudentText(student['Student_Name'] ?? student['name'] ?? 'Unknown Student')
  ...
  const yearLevel = yearLevelRaw ? repairStudentText(yearLevelRaw) : examYear ? `Exam ${repairStudentText(examYear)}` : '4th Year'
  const email = repairStudentText(student['Email'] ?? student['email'] ?? 'N/A')
...
export default function StudentPredictionsModule() {
  const PAGE_SIZE = 25
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setAllRows(analyticsRows.map((item, index) => toDisplayRow(item, index)))
        setCurrentPage(1)
      } catch (loadError) {
  ...
  const filtered = useMemo(() => {
    ...
    return allRows.filter((row) => {
      const matchesSearch = !normalized
        ...
        || normalizeForSearch(row.studentId).includes(normalized)

      const matchesRisk = riskFilter === 'ALL' || row.riskLevel === riskFilter
      return matchesSearch && matchesRisk
    })
  }, [allRows, riskFilter, search])

  useEffect(() => {
    setCurrentPage(1)
  }, [search, riskFilter, allRows])
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                {['STUDENT ID', 'NAME', 'YEAR LEVEL', 'GPA', 'PREDICTION', 'CONFIDENCE', 'ACTIONS'].map((header) => (
            ...
            <tbody>
              ...
              {loading && (
                <tr>
                  <td colSpan={7} className="px-8 py-10 text-sm text-gray-500 text-center">Loading predictions from Firestore...</td>
                </tr>
              )}

              {!loading && filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-8 py-10 text-sm text-gray-500 text-center">No prediction rows found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        {!loading && filtered.length > 0 && (
          <div className="flex items-center justify-between gap-3 border-t border-gray-200 px-6 py-4">
            <p className="text-sm text-gray-600">
              Showing {startIndex + 1}-{Math.min(startIndex + PAGE_SIZE, filtered.length)} of {filtered.length} records (page {currentPage} of {totalPages})
            </p>
